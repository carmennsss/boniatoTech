...
public class VistaGestorArchivos extends JFrame {
	...
	public VistaGestorArchivos() {
		this.setTitle("Gestión de archivos");
		this.setLayout(new BorderLayout());
		Font fuenteTitulo = new Font("Arial", Font.BOLD, 25);
		JLabel titulo = new JLabel("Archivos del servidor: ");
		titulo.setFont(fuenteTitulo);

		JButton botonSubida = new JButton("Subir fichero");
		JButton botonDescarga = new JButton("Descargar fichero");
		JButton botonEliminar = new JButton("Eliminar fichero");
		JButton botonCrearCarpeta = new JButton("Crear Carpeta");
		JButton botonBorrarCarpeta = new JButton("Borrar Carpeta");
	...
	public void accionBotonSubida(JButton boton) {
		...
		boton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				File file;
				fc.setDialogTitle("Seleccione el archivo a subir");
				fc.setFileSelectionMode(JFileChooser.FILES_ONLY);
				int respuesta = fc.showDialog(fc, "Aceptar");
	...
	public void accionBotonDescarga(JButton boton) {
		boton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				fc.setDialogTitle("Seleccione donde quieres descargar el archivo");
				...
				if (respuesta == JFileChooser.APPROVE_OPTION) {
					...
					if (select != null) {
						...
						JOptionPane.showMessageDialog(null, "Por favor, seleccione un archivo", "Error",
	...
	public void accionBotonCrearCarpeta(JButton boton) {
		boton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				nombreCarpeta = JOptionPane.showInputDialog(null, "Introduce el nombre del directorio", "");
				if (nombreCarpeta != null) {
					ftp.crearCarpeta(nombreCarpeta, rutaActual);
					actualizarListaFTP();
				} else {
					JOptionPane.showMessageDialog(null, "Por favor, escriba un nombre para la carpeta", "Error",
							JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
	...
	public void accionBotonBorrarCarpeta(JButton boton) {
		...
		boton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (select == null) {
					JOptionPane.showMessageDialog(null, "Por favor, seleccione una carpeta.", "Error",
							JOptionPane.INFORMATION_MESSAGE);
					return;
				}

				if (!select.isDirectory()) {
					JOptionPane.showMessageDialog(null, "Debe seleccionar una carpeta, no un archivo.", "Error",
							JOptionPane.INFORMATION_MESSAGE);
					return;
				}
	...
	public void actualizarListaFTP(String ruta) {
		...
		if (this.ftp.conectar()) {
			...
			JOptionPane.showMessageDialog(this, "No se pudo conectar al FTP");
		}
	}
