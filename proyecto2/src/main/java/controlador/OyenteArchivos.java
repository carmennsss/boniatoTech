...
public class OyenteArchivos implements ActionListener {
	...
	private void accionBotonRenombrar() {
		...
		String nuevoNombre = JOptionPane.showInputDialog(vista, "Enter new name for the file:", select.getName());
		if (nuevoNombre == null || nuevoNombre.trim().isEmpty()) {
			JOptionPane.showMessageDialog(vista, "Invalid name.", "Info", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
	...
	public boolean verificarPermiso(String ruta, String accion) {
		...
		String sqlPermisos = "SELECT * FROM usuarios_roles ur " + "JOIN roles_permisos rp ON ur.roles_id = rp.roles_id "
	...
	public void accionBotonSubida() {
		if (!verificarPermiso(rutaActual, "Subir archivos")) {
			JOptionPane.showMessageDialog(null, MoTextos.msg_permission_denied_upload,
					MoTextos.msg_permission_denied_title, JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	public void accionBotonBorrarCarpeta() {
		...
		if (!select.isDirectory()) {
			JOptionPane.showMessageDialog(null, MoTextos.msg_select_folder_not_file, MoTextos.msg_error_title,
					JOptionPane.INFORMATION_MESSAGE);
			return;
		}
